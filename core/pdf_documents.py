...
def _draw_brand_mark(c, x, y, size=28):
...
def render_booking_invoice_pdf(booking, settings, payment_totals, vat_amount=Decimal("0.00")):
    ...
    _draw_brand_mark(c, margin, y - 31, 28)
    ...
    c.setFillColor(GREEN if balance_due <= 0 else AMBER)
    ...
    for x, title in [(margin, "BILL TO"), (margin + box_w + 14, "BOOKING DETAILS")]:
        ...
        _draw_text(c, title, x + 12, y - 18, 8, GREEN, "Helvetica-Bold")
    ...
    for label, value, is_total in total_rows:
        if is_total:
            ...
            color = GREEN if label == "Balance due" and balance_due <= 0 else (AMBER if label == "Balance due" else TEXT)
    ...
    if settings.banking_details or settings.invoice_notes:
        ...
        if settings.banking_details:
            ...
            _draw_text(c, "PAYMENT DETAILS", margin + 10, notes_y - 15, 8, GREEN, "Helvetica-Bold")
        ...
        if settings.invoice_notes:
            ...
            _draw_text(c, "NOTES", x + 10, notes_y - 15, 8, GREEN, "Helvetica-Bold")
...
def render_payment_receipt_pdf(booking, payment, settings, receipt_number, balance_after_payment):
    ...
    _draw_text(c, "PAYMENT RECEIVED", margin + 18, y - 21, 9, GREEN, "Helvetica-Bold")
    ...
    if payment.notes or settings.receipt_notes:
        ...
        _draw_text(c, "NOTES", margin + 12, y - 16, 8, GREEN, "Helvetica-Bold")
...
def render_pos_receipt_pdf(sale, settings):
    width = 80 * mm
    height = 210 * mm
    buffer = BytesIO()
    ...
    _draw_text(c, settings.guest_house_name or "Circle Core Guest House", margin, y, 12, TEXT, "Helvetica-Bold")
    y -= 13
    y = _draw_wrapped(c, settings.address, margin, y, right - margin, 7.5, SLATE, leading=9, max_lines=2)
    if settings.phone:
        _draw_text(c, settings.phone, margin, y, 7.5, SLATE)
        y -= 10
    c.setStrokeColor(GREEN)
    c.line(margin, y, right, y)
    y -= 16
    _draw_text(c, "SALE RECEIPT", margin, y, 10, TEXT, "Helvetica-Bold")
    _draw_right(c, sale.sale_reference, right, y, 8, TEXT, "Helvetica-Bold")
    y -= 12
    _draw_text(c, sale.created_at.strftime("%d %b %Y %H:%M"), margin, y, 7.5, SLATE)
    y -= 14

    c.setFillColor(DARK)
    c.rect(margin, y - 14, right - margin, 14, fill=1, stroke=0)
    _draw_text(c, "ITEM", margin + 3, y - 10, 7, colors.white, "Helvetica-Bold")
    _draw_right(c, "TOTAL", right - 3, y - 10, 7, colors.white, "Helvetica-Bold")
    y -= 18
    for item in sale.items.all():
        _draw_wrapped(c, item.name, margin + 3, y, 120, 7.5, TEXT, "Helvetica-Bold", leading=9, max_lines=2)
        _draw_right(c, f"R{item.line_total:.2f}", right - 3, y, 7.5, TEXT, "Helvetica-Bold")
        y -= 18
    ...
    if sale.discount_amount > 0:
        _draw_text(c, "Discount", margin, y, 8, SLATE)
        _draw_right(c, f"-R{sale.discount_amount:.2f}", right, y, 8, TEXT)
        y -= 11
    if sale.tax_amount > 0:
        _draw_text(c, "VAT incl.", margin, y, 8, SLATE)
        _draw_right(c, f"R{sale.tax_amount:.2f}", right, y, 8, TEXT)
        y -= 11
    _draw_text(c, "TOTAL", margin, y, 11, TEXT, "Helvetica-Bold")
    _draw_right(c, f"R{sale.total:.2f}", right, y, 11, TEXT, "Helvetica-Bold")
    y -= 18
    c.setStrokeColor(GREEN)
    c.line(margin, y, right, y)
    y -= 12
    _draw_text(c, "Payment", margin, y, 8, SLATE)
    _draw_right(c, sale.get_payment_method_display(), right, y, 8, TEXT, "Helvetica-Bold")
    y -= 18
    _draw_text(c, "Thank you for your purchase.", margin, y, 8, SLATE)

    c.save()
    return _response(buffer, f"{sale.sale_reference}-receipt.pdf")
